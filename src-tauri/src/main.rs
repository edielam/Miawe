            ...
            Either::Left((event, _)) => match event.unwrap() {
                ...
                SwarmEvent::Behaviour(BehaviourEvent::Gossipsub(
                    ...
                    if message.topic == chat_topic_hash {
                        let received_message = String::from_utf8(message.data).unwrap();
                        info!("Received message from {:?}: {}", message.source, received_message);
                        shared_state.add_message(received_message);
                        continue;
                    }
...
#[tauri::command]
async fn send_message(message: String, state: State<'_, Arc<SharedState>>) -> Result<(), String> {
    // Send the message to the Gossipsub topic
    // Note: Ensure `swarm` is accessible here, perhaps via shared state
    // Example: `swarm.behaviour_mut().gossipsub.publish(chat_topic, message.as_bytes()).unwrap();`
    // For now, we will simulate sending the message by adding it to the shared state
